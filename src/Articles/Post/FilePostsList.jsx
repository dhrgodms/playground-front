...
const FilePostsList = () => {
    ...
    const tags = Object.entries(tagMapping).map(([id, info]) => ({
        ...
        icon: info.icon,
        color: info.color,
        count: filePosts.filter(post => post.categoryId === parseInt(id)).length
    }));

    useEffect(() => {
        axios
            .get(`${serverUrlV2}/posts?categoryId=4`)
            .then((response) => {
    ...
    return (
        <SubTemplate pageTitle={"파일 모음"} titleQuery={"파일"}>
            <Skeleton
                isLoaded={isLoaded}
                fadeDuration={1}
                startColor="#F8F9FA"
